...
export default class SpotlightCheck extends React.Component {
  ...
  start() {
    this.setStatePromise({ confetti_now:false, playing: true, ...this.getCoordinates('.playr-third'), placeText:'.playr-third'})
      ...
      .then(() => this.setStatePromise({ ...this.getCoordinates('.playr-third'), text:'במקום השלישי', radius: this.state.radiusNormal, color: 'rgb(216, 49, 25)', starting: true } ))
      .then(() => this.sleep(6000))
      .then(() => this.setStatePromise({ ...this.getCoordinates('.playr-second'), text:'במקום השני', color: 'rgb(192, 192, 192)',  placeText:'.playr-second' }))
      .then(() => this.sleep(6000))
      .then(() => this.setStatePromise({ ...this.getCoordinates('.playr-first'), text:'במקום הראשון', color: 'rgb(249, 194, 35)', radius: this.state.radiusWinner, winner: true,  placeText:'.playr-first'}))
